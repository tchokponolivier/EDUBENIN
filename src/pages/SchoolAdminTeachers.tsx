...
export function SchoolAdminTeachers() {
  ...
  const handleEditSubject = async (e: React.FormEvent) => {
     e.preventDefault();
     if (editingTeacherId && editingSubject.trim()) {
         await supabase.from('courses').update({ name: editingSubject }).eq('teacher_id', editingTeacherId);
              ...
              <div key={t.id} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
                <div className="p-4 border-b border-slate-100 flex items-center gap-4 bg-slate-50">
                  ...
                  <div className="flex items-center gap-1">
                    <button onClick={() => {
                        const tc = courses.filter(c => c.teacher_id === t.id); const subj = tc.length > 0 ? tc[0].name : "";
                        setEditingSubject(subj);
                        setEditingTeacherId(t.id);
                    }} className="p-1.5 text-blue-500 hover:bg-blue-100 rounded transition-colors" title="Modifier la matière">
                       <Edit2 size={16} />
                    </button>
         ...
         <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
            <div className="bg-white rounded-xl shadow-xl w-full max-w-sm overflow-hidden animate-in zoom-in-95">
               <div className="p-4 border-b border-slate-100 bg-slate-50">
                  <h3 className="font-bold text-gray-700">Modifier la matière enseignée</h3>
               </div>
               <form onSubmit={handleEditSubject} className="p-4 flex flex-col gap-4">
                  <div>
                  ...
                  <div className="flex justify-end gap-2">
                     ...
                     <button type="submit" className="px-4 py-2 text-sm font-bold bg-emerald-600 text-white rounded hover:bg-emerald-700">Enregistrer</button>
                  </div>
               </form>
            </div>
         </div>
