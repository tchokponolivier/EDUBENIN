  ...
  return (
    <div className="flex flex-col gap-6 p-6 animate-in fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-800">Espace Professeur</h1>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
         ...
         <div className="flex flex-col lg:flex-row gap-8 mb-6">
            ...
            <div className="flex-1 bg-slate-50 p-4 rounded-lg border border-slate-200">
               <h4 className="font-bold text-slate-700 mb-4 flex items-center gap-2"><BookOpen size={16}/> Matières du relevé</h4>
               {subjects.length > 0 ? (
                 <ul className="space-y-2 mb-4">
                   {subjects.map(s => (
                     <li key={s.id} className="flex items-center justify-between bg-white p-2 border border-slate-200 rounded text-sm">
                        <span className="font-semibold text-slate-800">{s.name} <span className="text-xs text-slate-500 font-normal">(Coef. {s.coef})</span></span>
            ...
            <div className="w-full lg:w-72 bg-slate-50 p-4 rounded-lg border border-slate-200 flex flex-col justify-between">
               <div>
                 <h4 className="font-bold text-slate-700 mb-2">Période</h4>
                 <input 
                    type="text" 
                    value={period} 
         ...
         <div className="overflow-x-auto print:overflow-visible">
            <h4 className="font-bold text-slate-700 mb-3 ml-1">Constitution des relevés {classStudents.length > 0 && `(${includedStudents.length}/${classStudents.length} sélectionnés)`}</h4>
                   ...
                   classStudents.map((student) => {
                     ...
                     return (
                       <tr key={student.id} className={`transition-colors ${isIncluded ? 'hover:bg-slate-50' : 'bg-slate-50/50 opacity-60'}`}>
                         ...
                         <td className="px-3 py-2 font-semibold text-slate-800 border border-slate-200 uppercase text-xs">
